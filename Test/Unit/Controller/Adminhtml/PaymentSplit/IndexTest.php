...
use Webjump\BraspagPagador\Controller\Adminhtml\PaymentSplit\Index;
...
class IndexTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->resultPageConfigMock->expects($this->exactly(1))
            ->method('getTitle')
            ->willReturn($this->resultPageTitleMock);

        $this->resultPageMock->expects($this->exactly(1))
            ->method('getConfig')
            ->willReturn($this->resultPageConfigMock);

        $this->resultPageMock->expects($this->exactly(3))
            ->method('addBreadcrumb')
            ->withConsecutive([__('Sales'), __('Sales')],[__('Braspag'), __('Braspag')],[__('Payment Splits'), __('Payment Splits')], [__('Index Payment Split'), __('Index Payment Split')])
            ->willReturnSelf();
